...
def fetch_directions(user, request_orig, request_dest, request_coord=""):
    """Fetch directions for a user from Google Maps.

    Arguments:
    - user: a user object
    - request_orig: REQUEST_TYPE_LOCATION, REQUEST_TYPE_HOME or REQUEST_TYPE_WORK
...
def parse_directions(directions_json):
    """Parse interesting info from a Google Maps response, return as a dict."""
    # Parse JSON
...
def get_directions(token_account):
    """Handle a directions request from a client."""
...
def get_next_local_time_occurence(target_time, now_local, timezone):
    """Get the next occurence of a time of day in the specified timezone."""
...
def send_pin(user, route, route_title, directions, departure_local, app_launch_code):
    """Send a trip pin to a user's Pebble timeline."""
...
def create_pin_regular(user, reason, user_config_version):
    """Create a scheduled regular pin, and schedule the next one of the same type."""
...
def schedule_next_pin_regular(user, reason, now_local, timezone):
    """Schedule the next regular pin for a user."""
    if reason == PIN_REASON_MORNING:
